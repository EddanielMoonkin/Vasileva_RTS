    ...
    CommandCreatorBase<TCommand> where TCommand : ICommand
{
    ...
    protected override sealed async void classSpecificCommandCreation(Action<TCommand> creactionCallback)
    {
        _ctSource = new CancellationTokenSource();
        try
        {
            var argument = await _awaitableArgument.WithCancellation(_ctSource.Token);
            creactionCallback?.Invoke(_context.Inject(createCommand(argument)));
        }
        catch { }
    }
